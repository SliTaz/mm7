'use strict';
var App = angular.module('onDemandDailyStatisticsModule', [ 'datatables',
...
function ServerSideCtrl(DTOptionsBuilder, DTColumnBuilder, $translate, $scope,
		$filter, $compile, OnDemandDailyStatisticsService, localStorageService,
		topleftService) {
	var vm = this;
	vm.modelTitle = "";
	vm.readonlyID = false;
	...
	function selData(){
		...
		OnDemandDailyStatisticsService.selSpInfo().then(function(d) {
		...
		OnDemandDailyStatisticsService.selUserInfo().then(function(d) {
		...
		OnDemandDailyStatisticsService.selProductInfo().then(function(d) {
