...
public class LogDao extends JdbcDaoSupport {
	...
	public void save(AbstractMessage message, String keyValue) {
		...
		int SERVUNIQUEID = 0;
		String servicecode = mtMsg.getServiceId()+"";
	...
	public void update(AbstractMessage message) {
		...
		String sql = "UPDATE USER_SERV_HISTORY set STATUS='" + mtMsg.getStatus() + "' WHERE MTTRANID='" + mtMsg.getCorrelator() + "'";
		logger.info(sql);
		try {
	...
	public void saveMoMMSMsg(AbstractMessage message, String SERVICEACTIVATIONNUMBER, Integer isCorrect) {
		...
		String sql = "INSERT INTO USER_MO_HISTORY (ID,CELLPHONENUMBER,SERVICEACTIVATIONNUMBER,SMSTEXT,MOTIME,MESSAGETYPE,isCorrect) VALUES(" + seqNextId + ",'" + str_phone + "','" + SERVICEACTIVATIONNUMBER
				+ "','彩信内容',{ts '" + this.formatDate.format(new Date()) + "'},'m','" + isCorrect + "')";
		try {
			getJdbcTemplate().update(sql);
		} catch (Exception e) {
			logger.error("保存上行消息出现异常", e);
	...
	public String getServicecode(String messageid) {
		...
		String service_code = (String) getJdbcTemplate().queryForObject(
				"select vas.SERVICEID from USER_SERV_HISTORY ush,VASSERVICE vass,VAS vas where ush.SERVUNIQUEID=vass.UNIQUEID and vass.VASID=vas.VASID and ush.MTTRANID=?", new Object[] { messageid }, String.class);
	...
	public void updateMoSmsMsg(int id, int iscorrect) {
		...
		getJdbcTemplate().update(sql, new String[] { iscorrect+"", id+"" });
	}
	...
	public void insert(MT_MMMessage msg) {
		...
		String sql = "insert into USER_SERV_HISTORY (MTTRANID,REQID,CELLPHONENUMBER,CHARGEPARTY,SERVUNIQUEID,CONTENTID,MMSFILE,MTTIME,STATUS,MTKIND) values ('" + msg.getMtTranId() + "','" + msg.getMtTranId() + "','"
				+ msg.getRcvAddresses()[0] + "','" + msg.getRcvAddresses()[0] + "',1,1,'" + msg.getServiceCode() + "',{ts '" + this.formatDate.format(new Date()) + "'},'2','2')";
		try {
			getJdbcTemplate().update(sql);
		} catch (Exception e) {
			e.printStackTrace();
