...
public class SmsProcess extends SmsHandler {
    ...
    private void thisLayerReceive(Object SM7_Cmd) throws InterruptedException {
        this.ComdQ.put(SM7_Cmd);
    }
    ...
    private int dealCommand(Object s) {
        SM7_Deliver deliver = (SM7_Deliver)s;
        ...
        switch(isReport) {
            ...
            case 1:
                ...
                byte[] id = deliver.getSMGW();
                SubmitKey msg = new SubmitKey((SM7_Submit)null, msgID);
                ...
                if (msgID != null) {
                    SM7_Deliver report = new SM7_Deliver(deliver, msgID);
    ...
    private void dealResp(Object s) {
        Object[] obj = (Object[])s;
        SM7_Submit ss = (SM7_Submit)obj[0];
        this.LogHandler.debug(ss.toString());
        int seq = ProduceSeq.getSeq();
        SM7_Submit ssubmit = new SM7_Submit(ss, seq);
    ...
    private void resendTask() {
        ...
        for(i = 0; i < list.size(); ++i) {
            this.UpHandler.send(new SM7_Submit(ByteBuffer.wrap((byte[])list.get(i))));
        }

        list = data.deliverResend();

        for(i = 0; i < list.size(); ++i) {
            this.DownHandler.send(new SM7_Deliver(ByteBuffer.wrap((byte[])list.get(i))));
        }
    ...
    private void deliverTimeout() {
        ...
        synchronized(deliverQueue) {
            ...
            while(it.hasNext()) {
                ...
                SM7_Deliver deliver = (SM7_Deliver)value[1];
    ...
    private void submitUnresp2DB() {
        ...
        if (unresp.length != 0) {
            ...
            for(int i = 0; i < unresp.length; ++i) {
                SM7_Submit submit = (SM7_Submit)unresp[i];
                int seq = submit.getSeqID();
    ...
    private void deliverUnresp2DB() {
        ...
        if (unresp.length != 0) {
            ...
            for(int i = 0; i < unresp.length; ++i) {
                SM7_Deliver deliver = (SM7_Deliver)unresp[i];
    ...
    private void submitresp2DB() {
        SM7_Submit[] resp = (SM7_Submit[])null;
        LinkedList var2 = this.submitResp;
        synchronized(this.submitResp) {
            resp = (SM7_Submit[])this.submitResp.toArray(new SM7_Submit[0]);
    ...
    private void deliverresp2DB() {
        SM7_Deliver[] resp = (SM7_Deliver[])null;
        LinkedList var2 = deliverResp;
        synchronized(deliverResp) {
            SM7_Deliver[] com = new SM7_Deliver[deliverResp.size()];
