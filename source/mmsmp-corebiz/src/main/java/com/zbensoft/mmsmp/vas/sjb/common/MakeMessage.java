 ...
 import com.zbensoft.mmsmp.vas.sjb.controller.DataCenter;
 import com.zbensoft.mmsmp.vas.sjb.util.DateString;
 import com.zbensoft.mmsmp.vas.sjb.util.ValidateOperators;
 ...
 public class MakeMessage
 {
   ...
   {
     List<String> mmsPhoneList = new LinkedList();
     List<String> mmePhoneList = new LinkedList();
     List<String> cm_mmsPhoneList = new LinkedList();
     List<String> te_mmsPhoneList = new LinkedList();
     List<String> uc_mmsPhoneList = new LinkedList();
     List<String> cm_smsPhoneList = new LinkedList();
     List<String> te_smsPhoneList = new LinkedList();
     List<String> uc_smsPhoneList = new LinkedList();
     List<String> cm_wapPhoneList = new LinkedList();
     List<String> te_wapPhoneList = new LinkedList();
     List<String> uc_wapPhoneList = new LinkedList();
     ...
     List<AbstractMessage> messageList = new LinkedList();
     Map<String, String> sysParamsMap = dataCenter.getSysParamsMap();
     Map<String, SysChannels> sysChannelMap = dataCenter.getSysChannelMap();
     LinkedList<UserInfo> userList = (LinkedList)userInfoList;
     ...
     if (vasservice.getProducttype().equalsIgnoreCase("m"))
     {
       if (isSameProduct) {
         ...
         while (userList.size() > 0) {
          UserInfo userInfo = (UserInfo)userList.getFirst();
           ...
           if (isAdapter) {
             ...
             ContentInfoRelation temp3=null;
             ...
             if (temp1 != null) {
               ...
               if ((area != null) && (contentInfo.getProvinceContent() != null)) {
            	  temp3 = (ContentInfoRelation)contentInfo.getProvinceContent().get(area);
                 if (temp3 != null) {
         ...
         while (userList.size() > 0) { UserInfo userInfo;
           String terminaltype; 
           ContentInfoAdapter temp1= null; 
           ContentInfoRelation temp2= null; 
           ContentInfoRelation temp3= null; 
           String[] address; 
           userInfo = (UserInfo)userList.getFirst();
           userList.removeFirst();
           String phoneNumber = userInfo.getCellphonenumber();
           if (isAdapter) {
             log.debug("需要适配!");
     ...
     if (vasservice.getProducttype().equalsIgnoreCase("e")) {
       ...
       if (isAdapter) {
         for (UserInfo userInfo : userInfoList)
         {
           ...
           if ((userInfo.getTerminaltype() != null) && 
             (contentInfo.getAdapterContentMap() != null)) {
             temp1 = (ContentInfoAdapter)contentInfo.getAdapterContentMap().get(
               Integer.valueOf(userInfo.getTerminaltype()));
           }
           
           if (temp1 != null) {
             if ((temp1.getProvinceContentMap() != null) && 
               (userInfo.getProvincecode() != null)) {
               temp2 = 
                 (ContentInfoRelation)temp1.getProvinceContentMap().get(userInfo.getProvincecode());
             }
             if (temp2 != null) {
               mmFile = temp2.getContentpath();
             } else {
           ...
           else if ((userInfo.getProvincecode() != null) && 
             (contentInfo.getProvinceContent() != null)) {
             ContentInfoRelation temp3 = 
               (ContentInfoRelation)contentInfo.getProvinceContent().get(userInfo.getProvincecode());
             if (temp3 != null) {
               mmFile = temp3.getContentpath();
             } else {
               mmFile = contentInfo.getContentpath();
           ...
           if (msg.getMmFile() == null) {
             ...
             if (size > 0) {
               String[] address = 
                 (String[])phoneList.toArray(new String[size]);
               msg.setRcvAddresses(address);
               messageList.add(msg);
             }
       ...
       else
       {
         ...
         msg.setMmFile(mmFile);
         LinkedList<UserInfo> usersList = (LinkedList)userInfoList;
         while (usersList.size() > 0) {
           for (int i = 0; i < te_mmeMultiNum;) {
             try {
       ...
       while (userList.size() > 0) {
         ...
         if (ValidateOperators.isChinaMobile(phoneNumber)) {
           ...
           if (cm_smsPhoneList.size() == cm_smsMultiNum) {
             MT_SMMessage smsg = new MT_SMMessage();
             smsg.setServiceId(vasservice.getUniqueid().intValue());
             smsg.setRcvAddresses((String[])cm_smsPhoneList.toArray(new String[cm_smsMultiNum]));
             if (cm_smsMultiNum > 1) {
               smsg.setGroupSend(true);
             }
             smsg.setOperatorsType(1);
             smsg.setMtTranId(streamingNum);
             smsg.setSendAddress(vasservice.getVasid());
             smsg.setServiceCode(vasservice.getServicecode());
             smsg.setVasId(vasservice.getVasid());
             smsg.setVaspId(vasservice.getVaspid());
             smsg.setTimeStamp(dateString);
             smsg.setProvinceCode(area);
             smsg.setContentid(contentInfo.getContentid());
             smsg.setSmsText(contentInfo.getSmsText());
             smsg.setMtUrl(cm_smsMtUrl);
           ...
           if (te_smsPhoneList.size() == te_smsMultiNum) {
             MT_SMMessage smsg = new MT_SMMessage();
             smsg.setServiceId(vasservice.getUniqueid().intValue());
             smsg.setRcvAddresses((String[])te_smsPhoneList.toArray(new String[te_smsMultiNum]));
             if (te_smsMultiNum > 1) {
               smsg.setGroupSend(true);
             }
             smsg.setOperatorsType(2);
             smsg.setMtTranId(streamingNum);
             smsg.setSendAddress(vasservice.getVasid());
             smsg.setServiceCode(vasservice.getServicecode());
             smsg.setVasId(vasservice.getVasid());
           ...
           if (uc_smsPhoneList.size() == uc_smsMultiNum) {
             MT_SMMessage smsg = new MT_SMMessage();
             smsg.setServiceId(vasservice.getUniqueid().intValue());
             smsg.setRcvAddresses((String[])uc_smsPhoneList.toArray(new String[uc_smsMultiNum]));
             if (te_smsMultiNum > 1) {
               smsg.setGroupSend(true);
             }
             smsg.setOperatorsType(3);
             smsg.setMtTranId(streamingNum);
             smsg.setSendAddress(vasservice.getVasid());
             smsg.setServiceCode(vasservice.getServicecode());
             smsg.setVasId(vasservice.getVasid());
       ...
       if (cm_smsPhoneList.size() > 0) {
         ...
         MT_SMMessage smsg = new MT_SMMessage();
         smsg.setServiceId(vasservice.getUniqueid().intValue());
         smsg.setRcvAddresses((String[])cm_smsPhoneList.toArray(new String[cm_smsPhoneList.size()]));
         if (cm_smsMultiNum > 1) {
           smsg.setGroupSend(true);
         }
         smsg.setOperatorsType(1);
       ...
       if (te_smsPhoneList.size() > 0) {
         ...
         MT_SMMessage smsg = new MT_SMMessage();
         smsg.setServiceId(vasservice.getUniqueid().intValue());
         smsg.setRcvAddresses((String[])te_smsPhoneList.toArray(new String[te_smsPhoneList.size()]));
         if (te_smsMultiNum > 1) {
           smsg.setGroupSend(true);
         }
         smsg.setOperatorsType(2);
       ...
       if (uc_smsPhoneList.size() > 0) {
         ...
         MT_SMMessage smsg = new MT_SMMessage();
         smsg.setServiceId(vasservice.getUniqueid().intValue());
         smsg.setRcvAddresses((String[])uc_smsPhoneList.toArray(new String[uc_smsPhoneList.size()]));
         if (te_smsMultiNum > 1) {
           smsg.setGroupSend(true);
         }
         smsg.setOperatorsType(3);
     ...
     if (vasservice.getProducttype().equalsIgnoreCase("p")) {
       while (userList.size() > 0) {
         ...
         if (ValidateOperators.isChinaMobile(phoneNumber)) {
           ...
           if (cm_wapPhoneList.size() == cm_wapMultiNum) {
             MT_WapPushMessage wapmsg = new MT_WapPushMessage();
             wapmsg.setServiceId(vasservice.getUniqueid().intValue());
             wapmsg.setRcvAddresses((String[])cm_wapPhoneList.toArray(new String[cm_wapMultiNum]));
             if (cm_wapMultiNum > 1) {
               wapmsg.setGroupSend(true);
             }
             wapmsg.setOperatorsType(1);
             wapmsg.setMtTranId(streamingNum);
             wapmsg.setSendAddress(vasservice.getVasid());
             wapmsg.setServiceCode(vasservice.getServicecode());
             wapmsg.setVasId(vasservice.getVasid());
             wapmsg.setVaspId(vasservice.getVaspid());
             wapmsg.setTimeStamp(dateString);
             wapmsg.setProvinceCode(area);
             wapmsg.setContentid(contentInfo.getContentid());
             wapmsg.setPushText(contentInfo.getContentname());
             wapmsg.setPushUrl(wappushURL + "?method=cDetail&cid=" + contentInfo.getContentid().toString());
             wapmsg.setMtUrl(cm_wapMtUrl);
             messageList.add(wapmsg);
           ...
           if (te_wapPhoneList.size() == te_wapMultiNum) {
             MT_WapPushMessage wapmsg = new MT_WapPushMessage();
             wapmsg.setServiceId(vasservice.getUniqueid().intValue());
             wapmsg.setRcvAddresses((String[])te_wapPhoneList.toArray(new String[te_wapMultiNum]));
             if (te_wapMultiNum > 1) {
               wapmsg.setGroupSend(true);
             }
             wapmsg.setOperatorsType(2);
             wapmsg.setMtTranId(streamingNum);
             wapmsg.setSendAddress(vasservice.getVasid());
             wapmsg.setServiceCode(vasservice.getServicecode());
             wapmsg.setVasId(vasservice.getVasid());
           ...
           if (uc_wapPhoneList.size() == uc_wapMultiNum) {
             MT_WapPushMessage wapmsg = new MT_WapPushMessage();
             wapmsg.setServiceId(vasservice.getUniqueid().intValue());
             wapmsg.setRcvAddresses((String[])uc_wapPhoneList.toArray(new String[uc_wapMultiNum]));
             if (uc_wapMultiNum > 1) {
               wapmsg.setGroupSend(true);
             }
             wapmsg.setOperatorsType(3);
             wapmsg.setMtTranId(streamingNum);
             wapmsg.setSendAddress(vasservice.getVasid());
             wapmsg.setServiceCode(vasservice.getServicecode());
             wapmsg.setVasId(vasservice.getVasid());
       ...
       if (cm_wapPhoneList.size() > 0) {
         ...
         MT_WapPushMessage wapmsg = new MT_WapPushMessage();
         wapmsg.setServiceId(vasservice.getUniqueid().intValue());
         wapmsg.setRcvAddresses((String[])cm_wapPhoneList.toArray(new String[cm_wapPhoneList.size()]));
         if (cm_wapMultiNum > 1) {
           wapmsg.setGroupSend(true);
         }
         wapmsg.setOperatorsType(1);
       ...
       if (te_wapPhoneList.size() > 0) {
         ...
         MT_WapPushMessage wapmsg = new MT_WapPushMessage();
         wapmsg.setServiceId(vasservice.getUniqueid().intValue());
         wapmsg.setRcvAddresses((String[])te_wapPhoneList.toArray(new String[te_wapPhoneList.size()]));
         if (te_wapMultiNum > 1) {
           wapmsg.setGroupSend(true);
         }
         wapmsg.setOperatorsType(2);
       ...
       if (uc_wapPhoneList.size() > 0) {
         ...
         MT_WapPushMessage wapmsg = new MT_WapPushMessage();
         wapmsg.setServiceId(vasservice.getUniqueid().intValue());
         wapmsg.setRcvAddresses((String[])uc_wapPhoneList.toArray(new String[uc_wapPhoneList.size()]));
         if (uc_wapMultiNum > 1) {
           wapmsg.setGroupSend(true);
         }
         wapmsg.setOperatorsType(3);
