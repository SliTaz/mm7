...
function ServerSideCtrl(DTOptionsBuilder, DTColumnBuilder, $translate, $scope,
    ...
    vm.columnStatusData=[];
    //列的状态end
    vm.dtOptions = DTOptionsBuilder.fromSource(getFromSource(apiUrl + '/contentInfo?status=2')).withOption(
...
//	}
	function edit(bean) {
		...
		vm.modelTitle = $translate.instant('common.testReview');
		vm.bean = bean;
		vm.readonlyID = true;
...
//	}
	...
	function reloadData() {
		...
		vm.dtInstance.changeData(getFromSource(apiUrl + '/contentInfo?contentName=' + getValueForSelect(contentName) + "&productInfoId="+ getValueForSelect(productInfoId) +  getValueForSelect(2,"&status=")
