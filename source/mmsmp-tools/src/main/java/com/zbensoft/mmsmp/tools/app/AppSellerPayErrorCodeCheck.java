...
public class AppSellerPayErrorCodeCheck {
	...
	public static void check() {
		...
		String toastUtilFile = "C:\\workspace\\android\\clap\\app\\src\\main\\java\\com\\huafu\\clap\\util\\ToastUtil.java";
		...
		for (int i = 0; i < lista.size(); i++) {
			...
			if (ispay) {
				if (!line.trim().startsWith("PAY_")) {
					System.err.println(AppSellerPayErrorCodeCheck.class.getSimpleName() + " e-payment-api HttpRestStatus defined error:" + line.trim());
				}
				String messageCode = line.trim().substring(0, line.trim().indexOf("("));
				if (!haveMessage(messageCode, listb)) {
					System.err.println(AppSellerPayErrorCodeCheck.class.getSimpleName() + " ToastUtil not process:" + line.trim());
				}
			}
		}
