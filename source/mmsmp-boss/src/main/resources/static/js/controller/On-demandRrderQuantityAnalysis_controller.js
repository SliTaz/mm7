'use strict';
var App = angular.module('OndemandRrderQuantityAnalysisModule', [ 'datatables',
...
function ServerSideCtrl(DTOptionsBuilder, DTColumnBuilder, $translate, $scope,
		$filter, $compile, OndemandRrderQuantityAnalysisService, localStorageService,
		topleftService) {
	var vm = this;
	vm.modelTitle = "";
	vm.readonlyID = false;
	...
	vm.dtColumns = [		
			...
			DTColumnBuilder.newColumn('newOrderTimes').withTitle($translate('userOrderHis.newOrderTimes')).notSortable()];
//	vm.addInit = addInit;
//	vm.edit = edit;
//	vm.submit = submit;
...
//	});
	...
	function selData(){
		...
		OndemandRrderQuantityAnalysisService.selProductInfo().then(function(d) {
...
//	}
	...
	function reloadData() {
		...
		vm.dtInstance.changeData(getFromSource(apiUrl
				...
				+ "&orderTimeEnd=" + getValueForSelect(orderTimeEnd)+ "&productInfoId=" + getValueForSelect(productInfoId) ));
