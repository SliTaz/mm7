...
public class SyncProductInfoHandleThread extends Thread {
    private static final Logger logger = Logger.getLogger(SyncProductInfoHandleThread.class);
    private static SyncProductInfoHandleThread _instance;
    private MyFtpTools ftp;
    private ProductInfoDAO productInfoDAO = new ProductInfoDAO();
    private SpInfoDAO spInfoDAO;
    long last_process_filestime = System.currentTimeMillis() - (long) (PropertiesHelper.getInt("last_process_filestime_productInfo") * 24 * 3600) * 1000L;

    public void setSpInfoDAO(SpInfoDAO spInfoDAO) {
        this.spInfoDAO = spInfoDAO;
    }

    public SyncProductInfoHandleThread() {
        ...
        String ftp_server_dir = PropertiesHelper.getString("ftp_product_server_dir");
        this.ftp = new MyFtpTools(ftp_server_ip, ftp_server_port, ftp_login_username, ftp_login_password, ftp_server_dir);
    }

    public static synchronized void startup() {
        if (_instance == null) {
            _instance = new SyncProductInfoHandleThread();
        }

        _instance.start();
        logger.info("SyncProductInfoHandleThread start...");
    }

    public void run() {
        while (true) {
            long ftp_monitor_interval = (long) PropertiesHelper.getInt("ftp_monitor_interval_productInfo");
            boolean ftp_monitor_thread_status = PropertiesHelper.getBoolean("ftp_product_isrunning");
            if (ftp_monitor_thread_status && this.downFtpFiles(PropertiesHelper.getString("ftp_product_server_dir"))) {
                this.handleFiles();
            }
    ...
    private List<File> getFileList() {
        File files = new File(PropertiesHelper.getString("ftp_product_local_dir"));
        File[] listFiles = files.listFiles(SyncProductInfoHandleThread.VacFileFilter.vff);
    ...
    private boolean downFtpFiles(String ftp_sp_server_dir) {
        String root_dir = PropertiesHelper.getString("ftp_product_local_dir");
    ...
    private void handleFile(File file) {
        ...
        try {
            List<ProductInfo> ors = ProductInfo.parseFile(file, spIdList);
            logger.info(ors.size());
            this.productInfoDAO.saveProductInfo(ors);
    ...
    private static class VacFileFilter implements FileFilter {
        static final SyncProductInfoHandleThread.VacFileFilter vff = new SyncProductInfoHandleThread.VacFileFilter();
        ...
        public boolean accept(File file) {
            ...
            if (!name.startsWith("proinfo") || !name.endsWith(".rreq") && !name.endsWith(".areq")) {
                SyncProductInfoHandleThread.logger.warn("handleFiles() - file name invalid, String name=" + name);
                return false;
            } else {
                return true;
            }
