'use strict';
var App = angular.module('userStatisticsModule', [ 'datatables',
...
function ServerSideCtrl(DTOptionsBuilder, DTColumnBuilder, $translate, $scope,
		$filter, $compile, UserStatisticsService, localStorageService,
		topleftService) {
	var vm = this;
	vm.modelTitle = "";
	vm.readonlyID = false;
	...
	vm.selectData = [];
	vm.appData = [];
	vm.dtOptions = DTOptionsBuilder.fromSource(getFromSource(apiUrl + '/userOrder/userStatistics')).withOption('createdRow', createdRow);

	setDtOptionsServerSide(vm);
	vm.dtColumns = [
			DTColumnBuilder.newColumn('time').withTitle($translate('userStatistics.time')).notSortable().notVisible(),
			DTColumnBuilder.newColumn('companyCode').withTitle($translate('userStatistics.companyCode')).notSortable(),
			DTColumnBuilder.newColumn('spInfoId').withTitle($translate('userStatistics.spInfoId')).notSortable(),
			DTColumnBuilder.newColumn('sendCount').withTitle($translate('userStatistics.sendCount')).notSortable(),
			DTColumnBuilder.newColumn('orderCount').withTitle($translate('userStatistics.orderCount')).notSortable(),
			DTColumnBuilder.newColumn('onDemandCount').withTitle($translate('userStatistics.onDemandCount')).notSortable(),
			DTColumnBuilder.newColumn('spTotalCost').withTitle($translate('userStatistics.spTotalCost')).notSortable(),
...
//	}
	...
	function selData(){
		UserStatisticsService.selSpInfo().then(function(d) {
	...
	function submit() {
		if (!vm.readonlyID) {
			...
			UserStatisticsService.createUserRecv(vm.bean).then(
			...
			UserStatisticsService.updateUserRecv(vm.bean,vm.bean.userRecvId).then(onSubmitSuccess,
					function(errResponse) {
						handleAjaxError(errResponse);
					...
					buttons : [
							{
								...
								action : function(dialogItself) {
									UserStatisticsService.deleteUserRecv(bean.userRecvId).then(reloadData,
													function(errResponse) {
														handleAjaxError(errResponse);
	...
	function reloadData() {
		...
		if(vm.beanSer.monthStart >= vm.beanSer.monthEnd){
			alert($translate.instant('userStatistics.monthRange'));
			return;
		}
		var monthStart = timeFormatNew(vm.beanSer.monthStart);
		var monthEnd = timeFormatNew(vm.beanSer.monthEnd);
		vm.dtInstance.changeData(getFromSource(apiUrl
				+ '/userOrder/userStatistics?spInfoId=' + getValueForSelect(spInfoId) + "&monthStart=" + getValueForSelect(monthStart)
				 + "&monthEnd=" + getValueForSelect(monthEnd)));
