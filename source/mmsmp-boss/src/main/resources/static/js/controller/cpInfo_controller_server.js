'use strict';
var App = angular.module('cpInfoModule', [ 'datatables',
...
function ServerSideCtrl(DTOptionsBuilder, DTColumnBuilder, $translate, $scope,
		$filter, $compile, CpInfoService, localStorageService,
		topleftService) {
	var vm = this;
	vm.modelTitle = "";
	vm.readonlyID = false;
	...
	vm.selectData = [];
	vm.appData = [];
	vm.dtOptions = DTOptionsBuilder.fromSource(getFromSource(apiUrl + '/cpInfo?type=1')).withOption('createdRow', createdRow);
	...
	vm.dtColumns = [
			...
			DTColumnBuilder.newColumn('cpInfoId').withTitle($translate('cpInfo.cpInfoId')).notSortable(),
			DTColumnBuilder.newColumn('status').withTitle($translate('cpInfo.status')).renderWith(statusDetail).notSortable(),
	...
	//初始化遍历spInfo
	function selData(){
		CpInfoService.selSpInfo().then(function(d) {
	...
	function addInit() {
		selectDevice();
		$("#spInfo2").val("").select2();
		vm.modelTitle = $translate.instant('cpInfo.add');
		vm.readonlyID = false;
		vm.disabled = false;
		vm.bean = {};
		vm.statusCode = "";
	...
	function edit(bean) {
		...
		vm.readonlyID = true;
		vm.modelTitle = $translate.instant('cpInfo.edit');
		vm.bean = bean;
	...
	function submit() {
		if (!vm.readonlyID) {
			...
			vm.bean.type = 1;
			CpInfoService.createCpInfo(vm.bean).then(
			...
			vm.bean.auditTime = timeFormatNew(vm.bean.auditTime);
			CpInfoService.updateCpInfo(vm.bean,vm.bean.cpInfoId).then(onSubmitSuccess,
					function(errResponse) {
						handleAjaxError(errResponse);
					...
					buttons : [
							{
								...
								action : function(dialogItself) {
									CpInfoService.deleteCpInfo(bean.cpInfoId).then(reloadData,
													function(errResponse) {
														handleAjaxError(errResponse);
	...
	function reloadData() {
		...
		vm.dtInstance.changeData(getFromSource(apiUrl
				+ '/cpInfo?cpInfoId=' + getValueForSelect(cpInfoId) + "&cpName=" + getValueForSelect(cpName)+ "&status="  + getValueForSelect(status)
				+ "&type="  + getValueForSelect(1)));
