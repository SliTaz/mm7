...
public class SmsProcess extends SmsHandler {
    ...
    private void thisLayerReceive(Object smgp_Cmd) throws InterruptedException {
        this.ComdQ.put(smgp_Cmd);
    }
    ...
    private int dealCommand(Object s) {
        SMGP_Deliver deliver = (SMGP_Deliver)s;
        ...
        switch(isReport) {
            ...
            case 1:
                ...
                SubmitKey msg = new SubmitKey((SMGP_Submit)null, msgID);
                Map var8 = submitQueue;
                Object[] value;
    ...
    private void dealResp(Object s) {
        Object[] obj = (Object[])s;
        SMGP_Submit ss = (SMGP_Submit)obj[0];
        int seq = ProduceSeq.getSeq();
        SMGP_Submit ssubmit = new SMGP_Submit(ss, seq);
    ...
    private void resendTask() {
        ...
        for(i = 0; i < list.size(); ++i) {
            this.UpHandler.send(new SMGP_Submit(ByteBuffer.wrap((byte[])list.get(i))));
        }

        list = data.deliverResend();

        for(i = 0; i < list.size(); ++i) {
            this.DownHandler.send(new SMGP_Deliver(ByteBuffer.wrap((byte[])list.get(i))));
        }
    ...
    private void deliverTimeout() {
        ...
        synchronized(deliverQueue) {
            ...
            while(it.hasNext()) {
                ...
                SMGP_Deliver deliver = (SMGP_Deliver)value[1];
    ...
    private void submitResend2DB() {
        ...
        if (unresp.length != 0) {
            ...
            for(int i = 0; i < unresp.length; ++i) {
                ...
                SMGP_Submit submit = (SMGP_Submit)obj[0];
                int seq = submit.getSeqID();
                ByteBuffer postBuf = submit.getBuf();
                Object[] value = (Object[])obj[1];
                ByteBuffer preBuf = ((SMGP_Submit)value[0]).getBuf();
                byte[] msgID = (byte[])value[1];
    ...
    private void submitUnresp2DB() {
        ...
        if (unresp.length != 0) {
            ...
            for(int i = 0; i < unresp.length; ++i) {
                SMGP_Submit submit = (SMGP_Submit)unresp[i];
                int seq = submit.getSeqID();
    ...
    private void deliverUnresp2DB() {
        ...
        if (unresp.length != 0) {
            ...
            for(int i = 0; i < unresp.length; ++i) {
                SMGP_Deliver deliver = (SMGP_Deliver)unresp[i];
    ...
    private void submitresp2DB() {
        SMGP_Submit[] resp = (SMGP_Submit[])null;
        LinkedList var2 = this.submitResp;
        synchronized(this.submitResp) {
            resp = (SMGP_Submit[])this.submitResp.toArray(new SMGP_Submit[0]);
    ...
    private void deliverresp2DB() {
        SMGP_Deliver[] resp = (SMGP_Deliver[])null;
        LinkedList var2 = deliverResp;
        synchronized(deliverResp) {
            SMGP_Deliver[] com = new SMGP_Deliver[deliverResp.size()];
