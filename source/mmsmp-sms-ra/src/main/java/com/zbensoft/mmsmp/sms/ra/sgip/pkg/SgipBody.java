...
public abstract class SgipBody {
    /**
     * 消息域列表
     */
    private List<SgipField> itemList = new LinkedList<SgipField>();

    public SgipBody(SgipField[] items) {
        itemList = new LinkedList<SgipField>();
     ...
     */
    private String autoFill(String value, char fillChar, SgipField.FillSide side, int totalLen, String charsetName) {
        try {
            if (value == null) {
                value = "";
            }
     ...
     * @throws Exception
     */
    private String getRealVal(String value, SgipField.FillSide side, char fillChar) throws Exception {
        char[] chars = value.toCharArray();
        if (SgipField.FillSide.LEFT == side) {
            int index = 0;
        ...
        } else if (SgipField.FillSide.RIGHT == side) {
