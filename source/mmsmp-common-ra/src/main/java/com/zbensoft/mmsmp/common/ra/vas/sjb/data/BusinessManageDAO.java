...
import com.zbensoft.mmsmp.common.ra.vas.sjb.unibusiness.dto.VasServiceDto;
...
public class BusinessManageDAO extends JdbcDaoSupport {
	...
	public boolean updateByProduct(Vasservice product) {
		try {
			String updateSql = "update vasservice set status = '" + product.getStatus() + "' where servicecode = '" + product.getServicecode() + "'";
			...
			if (Constants.ManageRequest_Operate_Cancel == Integer.valueOf(product.getStatus()).intValue()) {
				String delSql = "delete from user_order o where o.serviceuniqueid = (select uniqueid from vasservice v where v.servicecode = '" + product.getServicecode() + "')";
	...
	public boolean updateByVas(Vas vas) {
		try {
			String vasSql = "update vas v set v.status = " + vas.getStatus() + " where vasismpid = '" + vas.getVasIsmpId() + "'";

			log.debug("update vas:  " + vasSql);
			getJdbcTemplate().update(vasSql);
			log.debug("update vas information success!!!");

			String updateSql = "update vasservice set status = '" + vas.getStatus() + "' where vasnmscid = '" + vas.getVasIsmpId() + "'";
			...
			if (Constants.ManageRequest_Operate_Cancel == Integer.valueOf(vas.getStatus()).intValue()) {
				String delSql = "delete from user_order o where o.serviceuniqueid in (select uniqueid from vasservice v where v.vasnmscid = '" + vas.getVasIsmpId() + "')";
				log.debug("delete user_order relation" + delSql);
				getJdbcTemplate().update(delSql);
				log.debug("delete all user_order relation   success!!!");
			}
	...
	public boolean updateByCapacity(ServiceCapacity serviceCapacity) {
		try {
			String capSql = "update service_capacity set status = '" + serviceCapacity.getStatus() + "' where capacity_id = '" + serviceCapacity.getCapacityId() + "'";
			...
			String vasSql = "update vas v set v.status = " + serviceCapacity.getStatus() + " where capacity_id =  '" + serviceCapacity.getCapacityId() + "'";

			log.debug("update vas status ：" + vasSql);
			getJdbcTemplate().update(vasSql);
			log.debug("update vas status success.");

			String updateSql = "update vasservice set status = '" + serviceCapacity.getStatus() + "' where vasnmscid in (select vasnmscid from vas va where va.capacity_id =  '" + serviceCapacity.getCapacityId() + "')";
	...
	public List<Vasservice> findNormalStatus() {
		try {
			String queryString = "select a.uniqueid,a.vaspid,a.vasid,a.servicecode,a.feetype,a.orderfee,a.ispackage as isPackage,a.product_type,a.servicename,a.ondemandfee,a.ordercode,a.cancelordercode,a.ondemandcode,b.recordsequenceid,b.spec_productid,b.sp_productid,b.confirm,b.presenter,b.confirmprompt,b.successprompt,b.cancelprompt,b.popularizestart,b.popularizestop,b.freeuseflag,b.freeusetime,b.billingid,b.discount_des,b.discountid,b.needconfm,b.maxusetimes,b.maxusetime,b.sendnum,b.ordercmdmatch,b.orderacc,b.orderaccmatch,b.cancelcmdmatch,b.cancelacc,b.cancelaccmatch,b.requestcmdmatch,b.requestacc,b.requestaccmatch,b.effdate,b.expdate,b.vacsub,b.notifytype,b.productcity,b.productperiodgrade,b.productservicegrade,b.productcredit,b.productstatus,b.productid,b.product_extend_id,b.product_expand_code,b.report_message_url from vasservice a, vasservice_reserve_info b where a.servicecode = b.productid and   a.status ='2' and a.dealStatus = 0 or a.dealStatus =1";
	...
	public String getPlatFormModel() {
		...
		try {
			...
			e.printStackTrace();
			this.log.error("get content status error! sql: " + sql, e);
		} finally {
			releaseConnection(conn);
			if (preStmt != null) {
				try {
	...
	public boolean updateProductDealStatus(Integer dealStatus, List<Vasservice> list) {
		...
		try {
			...
			e.printStackTrace();
			this.log.error("set dealstatus error! sql: " + sql, e);
		} finally {
			releaseConnection(conn);
			if (preStmt != null)
				try {
