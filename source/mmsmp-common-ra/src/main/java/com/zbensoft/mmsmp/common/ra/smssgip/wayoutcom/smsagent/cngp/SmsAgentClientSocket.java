...
public class SmsAgentClientSocket extends ClientSocket {
    ...
    public int sendtoSMS(Object objSMS) {
        ...
        byte nBytes;
        try {
            CNGP_Deliver deliver = (CNGP_Deliver)objSMS;
    ...
    private boolean readResp(Socket sc) {
        try {
            CNGP_DeliverResp resp = (CNGP_DeliverResp)CNGP_Command.read(sc);
            if (resp == null) {
                Thread.sleep(1000L);
                return true;
            } else {
    ...
    private int dealResp(CNGP_DeliverResp resp) {
        int result = resp.getStatus();
        if (result == 0) {
            int seq = resp.getSeqID();
            CNGP_Command value = null;
    ...
    private Socket refreshSocket(CNGP_Deliver command) {
