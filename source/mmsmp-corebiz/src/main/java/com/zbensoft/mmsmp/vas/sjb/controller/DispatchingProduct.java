   ...
   implements Runnable
 {
   ...
   public DispatchingProduct(MessageQueue queue, List<ContentInfo> contentList, Vasservice vasservice, List<String> areaList, LoadUserOrderDao loadUserOrderDao, ContentInfoDAO contentInfoDAO, BusinessManageDAO businessManageDAO, DataCenter dataCenter)
   {
     this.queue = queue;
   ...
   public void run()
   {
     try
     {
       List<String> cyAreaList = new ArrayList();
   ...
   private void loadData(int priority, List<String> cyAreaList)
   {
     ...
     while (cyAreaList.size() > 0)
     {
       ...
       List<UserInfo> userInfos = this.loadUserOrderDao.getUserOrderRecord(
         this.vasservice.getUniqueid(), Integer.valueOf(dbReqNum), this.batchID, area, Integer.valueOf(priority), this.contenIDString, this.isNational);
       ...
       else
       {
         ...
         for (Iterator localIterator1 = this.contentList.iterator(); localIterator1.hasNext();){
           ...
           {
             if (this.contentList.size() != 1) break;
             return;
           }
