...
public class DemandDao extends JdbcDaoSupport {
	private static Logger logger = Logger.getLogger(DemandDao.class);
	private DecimalFormat format = new DecimalFormat("#####.00");

	public String getNewmessageid(String MTTRANID) {
		String messageid = "";
		String sql = "select MESSAGEID from USER_SERV_HISTORY where MTTRANID='" + MTTRANID + "'";
		logger.info("=====>" + sql);
	...
	public void updateSpMMSSendRecord(String status, String mttranid) {
		String sql = "update USER_SERV_HISTORY set status = '" + status + "' where mttranid = '" + mttranid + "'";
		logger.info("sql:" + sql);
		try {
	...
	public String queryOndemandMessage() {
		String sql = "select value from SYS_PARAMETERS sys where sys.NAME='ONDEMAND_TIP'";
	...
	public Userservhistory getUserservhistoryBymttranid(String mttranid) {
		String sql = "select * from USER_SERV_HISTORY where MTTRANID='" + mttranid + "'";
		logger.info("=====>sql:" + sql);
		try {
			Object object = getJdbcTemplate().queryForObject(sql, new RowMapper() {
				public Object mapRow(ResultSet rs, int arg1) throws SQLException {
	...
	public String getSp_url(String MTTRANID) {
		String url = "";
		String sql = "select va.reportmessageurl from VASP_RESERVE_INFO va,USER_SERV_HISTORY us where va.spid=us.spid and us.MTTRANID='" + MTTRANID + "'";
		logger.info("=====>" + sql);
		List list = getJdbcTemplate().query(sql, new RowMapper() {
			public Object mapRow(ResultSet rs, int arg1) throws SQLException {
				return rs.getString(1);
	...
	public Object[] judgeAnswer(String phone, String moText, String delim) {
		...
		List results = getJdbcTemplate().query(sql, new Object[] { Integer.valueOf(codeLength), code }, new RowMapper() {
			public Object mapRow(ResultSet rs, int arg1) throws SQLException {
	...
	public boolean isReplied4Twice(String phone, String code, String delim, Date beginDate, Date endDate) {
		...
		for (int i = 0; i < results.size(); i++) {
			...
			if ((motime != null) && (compareDate(motime, beginDate) >= 0) && (compareDate(motime, endDate) <= 0)) {
				return true;
			}
	...
	public String getSp_urlbySpid(String spid) {
		...
		String url = (String) getJdbcTemplate().queryForObject("select reportmessageurl from VASP_RESERVE_INFO where spid=?", new Object[] { spid }, String.class);
		return url;
	}

	public DemandDto getDemandDto(String sendto) {
		String sql = "select vass.vaspid,vas.SERVICEID,vasri.SP_PRODUCTID,vasp.REPORTMESSAGEURL,vasri.NEEDCONFM from VAS vas,VASSERVICE vass,VASSERVICE_RESERVE_INFO vasri,VASP_RESERVE_INFO vasp where vass.SERVICECODE=vasri.PRODUCTID and vas.VASID=vass.VASID and vasp.spid=vass.vaspid and (vasri.ORDERACC='"
				+ sendto + "' or vass.vasid='" + sendto + "')";
		logger.info("=====>sql:" + sql);
		try {
