'use strict';
var App = angular.module('orderMonthStatisticsModule', [ 'datatables',
...
function ServerSideCtrl(DTOptionsBuilder, DTColumnBuilder, $translate, $scope,
		$filter, $compile, OrderMonthStatisticsService, localStorageService,
		topleftService) {
	var vm = this;
	vm.modelTitle = "";
	vm.readonlyID = false;
	...
	function selData(){
		...
		OrderMonthStatisticsService.selSpInfo().then(function(d) {
		...
		OrderMonthStatisticsService.selUserInfo().then(function(d) {
		...
		OrderMonthStatisticsService.selProductInfo().then(function(d) {
