...
public class RequestXmlHandler {
	...
	public String getNodeValue(String str, String start_pattern, String end_pattern) {
		if ((start_pattern == null) || (end_pattern == null))
			throw new IllegalArgumentException("node_pattern cannot be null");
		...
		if (matcher_start.find()) {
			...
			Matcher matcher_end = Pattern.compile("</([a-zA-Z0-9_]*:)?" + end_pattern + ">", 2)
					.matcher(str.substring(start));
			if (matcher_end.find()) {
	...
	public void initlizeClass(String bottom_scope, Object o) throws Exception {
		if (o == null)
			throw new IllegalArgumentException("object cannnot be null");
