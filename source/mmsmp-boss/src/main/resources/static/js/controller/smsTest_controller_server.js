'use strict';
var App = angular.module('smsTestModule', [ 'datatables',
...
function ServerSideCtrl(DTOptionsBuilder, DTColumnBuilder, $translate, $scope,
		$filter, $compile, SmsTestService, localStorageService,
		topleftService) {
	var vm = this;
	vm.modelTitle = "";
	vm.readonlyID = false;
					...
					buttons : [
							{
								...
								action : function(dialogItself) {
									...
									//发送方法
									SmsTestService.smsTest(vm.bean).then(function(d) {
//											vm.smsData = d.body;
										});
									dialogItself.close();
								}
					...
					BootstrapDialog.show({
			            ...
			            onshown: function(dialogRef){//打开完成
			            	...
			            	var uploader = WebUploader.create({
			            		...
			            		auto: true,
			            	    // 文件接收服务端。
			            	    server: apiUrl+'/messageTest/smsTest/Import?sendContent=' + vm.bean.sendContent,
			            	    method: 'POST',
					...
					buttons : [
							{
								...
								cssClass : 'btn btn-danger model-tool-right',
								action : function(dialogItself) {
									SmsTestService.deleteUserRecv(bean.userRecvId).then(reloadData,
