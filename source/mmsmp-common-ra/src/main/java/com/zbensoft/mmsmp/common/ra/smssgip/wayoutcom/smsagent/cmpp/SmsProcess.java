...
public class SmsProcess extends SmsHandler {
    ...
    public static volatile int test1 = 0;
    public static volatile int writeError = 0;
    ...
    private int dealCommand(Object s) {
        --testCount;
        CMPP_Deliver deliver = (CMPP_Deliver)s;
        ...
        switch(isReport) {
            ...
            case 1:
                ...
                SubmitKey msg = new SubmitKey((CMPP_Submit)null, new Long(msgID));
                ...
                if (msgID != 0L) {
                    CMPP_Deliver report = new CMPP_Deliver(deliver, msgID);
    ...
    private void dealResp(Object s) {
        ++testCount;
        Object[] obj = (Object[])s;
        CMPP_Submit ss = (CMPP_Submit)obj[0];
        int seq = ProduceSeq.getSeq();
        CMPP_Submit ssubmit = new CMPP_Submit(ss, seq);
        SubmitKey key = new SubmitKey(ssubmit, (Long)null);
    ...
    private void resendTask() {
        ...
        for(i = 0; i < list.size(); ++i) {
            this.UpHandler.send(new CMPP_Submit(ByteBuffer.wrap((byte[])list.get(i))));
        }

        list = data.deliverResend();

        for(i = 0; i < list.size(); ++i) {
            this.DownHandler.send(new CMPP_Deliver(ByteBuffer.wrap((byte[])list.get(i))));
        }
    ...
    private List collectTimeout(int timeout, boolean flag) {
        ...
        synchronized(submitQueue) {
            ...
            while(it.hasNext()) {
                ...
                boolean con = true;
                if (con) {
                    long msgID = (Long)value[1];
                    int[] time = new int[]{ProduceMsgID.getMinute(msgID), ProduceMsgID.getHour(msgID), ProduceMsgID.getDay(msgID), ProduceMsgID.getMonth(msgID)};
                    int totalMin = 0;
                    int exp = 1;
                    int len = time.length;
                    ...
                    now = (cal.get(2) + 1) * 30 * 24 * 60;
                    now += cal.get(5) * 24 * 60;
                    now += cal.get(11) * 60;
                    now += cal.get(12);
                    int differ = now - totalMin;
                    if (differ <= timeout) {
                        break;
                    }
    ...
    private void deliverTimeout() {
        ...
        synchronized(deliverQueue) {
            ...
            while(it.hasNext()) {
                ...
                CMPP_Deliver deliver = (CMPP_Deliver)value[1];
    ...
    private void submitResend2DB() {
        ...
        if (unresp.length != 0) {
            ...
            for(int i = 0; i < unresp.length; ++i) {
                ...
                CMPP_Submit submit = (CMPP_Submit)obj[0];
                int seq = submit.getSeqID();
                ByteBuffer postBuf = submit.getBuf();
                Object[] value = (Object[])obj[1];
                ByteBuffer preBuf = ((CMPP_Submit)value[0]).getBuf();
                long msgID = (Long)value[1];
                Object[] object = new Object[]{postBuf, preBuf, new Integer(seq), new Long(msgID)};
                list.add(object);
            }
    ...
    private void submitUnresp2DB() {
        ...
        if (unresp.length != 0) {
            ...
            for(int i = 0; i < unresp.length; ++i) {
                CMPP_Submit submit = (CMPP_Submit)unresp[i];
                int seq = submit.getSeqID();
    ...
    private void deliverUnresp2DB() {
        ...
        if (unresp.length != 0) {
            ...
            for(int i = 0; i < unresp.length; ++i) {
                CMPP_Deliver deliver = (CMPP_Deliver)unresp[i];
    ...
    private void submitresp2DB() {
        CMPP_Submit[] resp = (CMPP_Submit[])null;
        LinkedList var2 = this.submitResp;
        synchronized(this.submitResp) {
            resp = (CMPP_Submit[])this.submitResp.toArray(new CMPP_Submit[0]);
    ...
    private void deliverresp2DB() {
        CMPP_Deliver[] resp = (CMPP_Deliver[])null;
        LinkedList var2 = deliverResp;
        synchronized(deliverResp) {
            CMPP_Deliver[] com = new CMPP_Deliver[deliverResp.size()];
