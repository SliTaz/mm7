...
public class DataCenter {
	...
	public void refreshVASService() {
		List servList = this.businessManageDAO.findNormalStatus();
		Vasservice serv = null;
		Map serviceMapById = new HashMap();
	...
	public void refreshSP() {
		List servList = this.businessManageDAO.findSpInfo();
		Vasp vasp = null;
		Map spMapById = new HashMap();
		for (int i = 0; i < servList.size(); i++) {
	...
	public void refreshWhiteUser() {
		List userList = this.businessManageDAO.findWhiteUserInfo();
		UserBlackWhiteList user = null;
		Map whiteUserMapById = new HashMap();
		for (int i = 0; i < userList.size(); i++) {
			user = (UserBlackWhiteList) userList.get(i);
			whiteUserMapById.put(user.getProductId(), user);
		}
		this.whiteUserMapById = whiteUserMapById;
	}

	public void refreshVas() {
		List vasList = this.businessManageDAO.findVasInfo();
		Vas vas = null;
		Map vasMapById = new HashMap();
	...
	public void refreshServMTMode() {
		Map servMtModeMap = new HashMap();
		List servMtModes = this.businessManageDAO.findAllServMod();
	...
	public ServMtMode findServMtMode(ServMtModeId servMtModeId) {
		return (ServMtMode) this.servMtModeMap.get(servMtModeId);
	}
	...
	public void refreshSysParams() {
		Map sysParamsMap = new HashMap();
		List sysParams = this.businessManageDAO.findAllSysParams();
	...
	public void refreshSysChannel() {
		Map sysChannelMap = new HashMap();
		String area = (String) this.sysParamsMap.get("PLATFORM_AREA");
		List sysChannel = this.businessManageDAO.findAllChannel(area.split("@")[0]);
	...
	public void setServiceMapById(Map<Integer, Vasservice> serviceMapById) {
		this.serviceMapById = serviceMapById;
	}
	...
	public void setServMtModeMap(Map<ServMtModeId, ServMtMode> servMtModeMap) {
		this.servMtModeMap = servMtModeMap;
	}
