...
public class MerchantCheckJson {

	public static void check() {
		String path = System.getProperty("user.dir").replaceAll("e-payment-tools", "e-payment-merchant") + "\\src\\main\\resources\\static\\il8n\\";
		...
		for (int i = 1; i < files.length; i++) {
			...
			for (Iterator<String> iterator = jsonObject.keySet().iterator(); iterator.hasNext();) {
				...
				if (!jsonObjectOther.keySet().contains(key)) {
					System.err.println(String.format("%s %s key not in %s : key= %s", MerchantCheckJson.class.getSimpleName(), files[0], files[i], key));
				}
			}
			...
			for (Iterator<String> iterator = jsonObjectOther.keySet().iterator(); iterator.hasNext();) {
				...
				if (!jsonObject.keySet().contains(key)) {
					System.err.println(String.format("%s %s key not in %s : key= %s", MerchantCheckJson.class.getSimpleName(), files[i], files[0], key));
				}
			}
		}
	}
