...
public class SyncNotifySPSoapBindingImpl implements SyncNotifySPService {
	...
	public OrderRelationUpdateNotifyResponse orderRelationUpdateNotify(
		...
		if ((orderRelationUpdateNotifyRequest.getProductId() != null)
				&& (orderRelationUpdateNotifyRequest.getProductId().trim().length() > 0))
		...
		try {
			...
			UniBusiness uniBusiness = client
					.getUniBusinessService(ConfigUtil.getInstance().getCommonConfig().getUniBusinessUrl());
	...
	private void dispatchMessage(OrderRelationUpdateNotifyRequest orderRelationUpdateNotifyRequest)
		...
		if ((url != null) && (!"".equals(url))) {
			...
			OrderRelationUpdateNotifyResponse value = binding
					.orderRelationUpdateNotify(orderRelationUpdateNotifyRequest);
