...
function ServerSideCtrl(DTOptionsBuilder, DTColumnBuilder, $translate, $scope,
    ...
    vm.columnStatusData=[];
    //列的状态end
    vm.dtOptions = DTOptionsBuilder.fromSource(getFromSource(apiUrl + '/contentInfo?status=3')).withOption(
...
//	}
	function edit(bean) {
		...
		vm.modelTitle = $translate.instant('common.sendReview');
		vm.bean = bean;
		vm.readonlyID = true;
...
//	}
	...
	function reloadData() {
		...
		vm.dtInstance.changeData(getFromSource(apiUrl + '/contentInfo?contentName=' + getValueForSelect(contentName) + "&productInfoId="+ getValueForSelect(productInfoId) +  getValueForSelect(3,"&status=")
