'use strict';
var App = angular.module('serviceReportModule', [ 'datatables',
...
function ServerSideCtrl(DTOptionsBuilder, DTColumnBuilder, $translate, $scope,
		$filter, $compile, ServiceReportService, localStorageService,
		topleftService) {
	var vm = this;
	vm.modelTitle = "";
	vm.readonlyID = false;
	...
	vm.dtOptions = DTOptionsBuilder.fromSource(getFromSource(apiUrl + '/userOrderHis/ByMonth?orderTimeStart=' + getValueForSelect(weekTime)+"&orderTimeEnd="+getValueForSelect(todayTime))).withOption('createdRow', createdRow);
	...
	vm.dtColumns = [
			...
			DTColumnBuilder.newColumn('orderTimes').withTitle($translate('userServiceSend.orderTimes')).notSortable(),
			DTColumnBuilder.newColumn('onDemandFee').withTitle($translate('userOrderHis.onDemandFee')).notSortable(),
			DTColumnBuilder.newColumn('totalMonth').withTitle($translate('userOrderHis.TotalMonth')).notSortable(),
			DTColumnBuilder.newColumn('ratioByNearMonth').withTitle($translate('userOrderHis.ratioByNearMonth')).notSortable(),
			DTColumnBuilder.newColumn('ratioByNextYear').withTitle($translate('userOrderHis.ratioByNextYear')).notSortable()];
//	vm.addInit = addInit;
//	vm.edit = edit;
//	vm.submit = submit;
...
//	});
	...
	function selData(){
		...
		ServiceReportService.selSpInfo().then(function(d) {
		...
		ServiceReportService.selProductInfo().then(function(d) {
...
//	}
	...
	function reloadData() {
		...
		vm.dtInstance.changeData(getFromSource(apiUrl
				+ '/userOrderHis/ByMonth?orderTimeStart=' + getValueForSelect(orderTimeStart)
