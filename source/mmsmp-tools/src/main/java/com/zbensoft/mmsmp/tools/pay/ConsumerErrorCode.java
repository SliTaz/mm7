...
public class ConsumerErrorCode {
	...
	public static void check() {
		...
		String file = System.getProperty("user.dir").replaceAll("e-payment-tools", "e-payment-consumer") + "\\src\\main\\resources\\static\\errorCode.json";
		...
		for (int i = 0; i < lista.size(); i++) {
			...
			if (ispay) {
				...
				if (!line.trim().startsWith("PAY_")) {
					System.err.println(ConsumerErrorCode.class.getSimpleName() + " e-payment-api HttpRestStatus defined error:" + line.trim());
				}
				String messageCode = line.trim().substring(0, line.trim().indexOf("("));
				if (!haveMessage(messageCode, listb)) {
					System.err.println(ConsumerErrorCode.class.getSimpleName() + " errorCode.json not process:" + line.trim());
				}
			}
		}
