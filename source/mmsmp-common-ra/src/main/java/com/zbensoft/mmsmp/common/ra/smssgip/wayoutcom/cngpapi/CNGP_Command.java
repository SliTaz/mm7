...
public abstract class CNGP_Command {
    ...
    public static CNGP_Command read(Socket sc) throws IOException, Exception {
            ...
            if (pac_len >= head.length && pac_len <= 65536) {
                ...
                if (count < body.length) {
                    ...
                    switch(TypeConvert.byte2int(packet, 4)) {
                        case -2147483647:
                            return new CNGP_BindResp(ByteBuffer.wrap(packet));
                        case -2147483646:
                            return new CNGP_SubmitResp(ByteBuffer.wrap(packet));
                        case -2147483645:
                            return new CNGP_DeliverResp(ByteBuffer.wrap(packet));
                        case -2147483644:
                            return new CNGP_ActiveTestResp(ByteBuffer.wrap(packet));
                        case -2147483642:
                            return new CNGP_UnbindResp(ByteBuffer.wrap(packet));
                        case 1:
                            return new CNGP_Bind(ByteBuffer.wrap(packet));
                        case 2:
                            return new CNGP_Submit(ByteBuffer.wrap(packet));
                        case 3:
                            return new CNGP_Deliver(ByteBuffer.wrap(packet));
                        case 4:
                            return new CNGP_ActiveTest(ByteBuffer.wrap(packet));
                        case 6:
                            return new CNGP_Unbind(ByteBuffer.wrap(packet));
                        default:
                            return null;
                    }
                }
