 ...
 import com.zbensoft.mmsmp.vas.sjb.common.MakeMessage;
 ...
 public class SendNotificationMessageHandler
   extends AbstractMessageHandler
 {
   ...
   public void handleMessage(Object notifyMessage)
   {
     ...
     try
     {
       ...
       AbstractMessage msg; if (notifyMsg.getSendType() == 1) {
         ...
         if (notifyMsg.getServiceType() == 1)
         {
           ...
           List<ContentInfo> contentInfo = this.contentInfoDao.getContentByContentId(notifyMsg.getContentId(), false);
           logger.info("<<<<<<<<<<<<<<<<<< dianbo");
           if ((contentInfo != null) && (contentInfo.size() > 0)) {
             ...
             List<UserInfo> userList = this.loadUserOrderDao.getUserInfoForDemand(notifyMsg.getPhoneNumber(), mtranId, content.getContentid().intValue(), vasService.getUniqueid().intValue(), notifyMsg.getMtType());
             if (userList.size() <= 0) {
             ...
             for (Iterator localIterator1 = mtMessageList.iterator(); localIterator1.hasNext();) { msg = (AbstractMessage)localIterator1.next();
         ...
         else
         {
           ...
           {
             String mtranId = MakeMessage.generateStreamingNum(Integer.toString(vasService.getUniqueid().intValue()));
             List<UserInfo> userList = this.loadUserOrderDao.getUserInfo(vasService.getUniqueid(), ((ContentInfo)contentInfo.get(0)).getContentid(), mtranId, notifyMsg.getPhoneNumber(), notifyMsg.getMtType());
             try
             {
               List<AbstractMessage> mtMessageList = MakeMessage.getMessage(vasService, (ContentInfo)contentInfo.get(0), userList, null, mtranId, this.dataCenter);
               
               for (AbstractMessage absmsg : mtMessageList) {
                 this.MT_queue.addMessage(absmsg, Integer.valueOf(MessageQueue.PRIORITY_VIP));
               }
             } catch (Exception e) {
               e.printStackTrace();
             }
         ...
         List<ContentInfo> contentInfo = this.contentInfoDao.getNeedSendContentList(vasService.getUniqueid().intValue(), false, false);
         if ((contentInfo != null) && (contentInfo.size() > 0)) {
...
//           AbstractMessage localAbstractMessage1 = (arrayOfString1 = province).length; 
//           for (int i = 0; i < localAbstractMessage1; i++) { 
//        	   String area = arrayOfString1[i];
//             provinceList.add(area);
//           }
           String[] arrayOfString1=province; 
           ...
           while (provinceList.size() > 0) {
             ...
             Object userList = this.loadUserOrderDao.getUserOrderRecordBySendStatus(vasService.getUniqueid(), ((ContentInfo)contentInfo.get(0)).getContentid(), reqNum, mtranId, area, notifyMsg.getStatus(), notifyMsg.getMtType());
             if (((List)userList).size() == 0) {
               provinceList.remove(k);
             }
             else {
               if (((List)userList).size() < reqNum) {
                 provinceList.remove(k);
               }
               
               List<AbstractMessage> mtMessageList = MakeMessage.getMessage(vasService, (ContentInfo)contentInfo.get(0), (List)userList, null, mtranId, this.dataCenter);
         ...
         this.MT_queue.addMessage(mtMessage, Integer.valueOf(MessageQueue.PRIORITY_LOW));
       }
     } catch (Exception e) {
       e.printStackTrace();
   ...
   {
     String mtranId = MakeMessage.generateStreamingNum(Integer.toString(vasService.getUniqueid().intValue()));
     List<UserInfo> userList = this.loadUserOrderDao.getUserOrderRecord(vasService.getUniqueid(), Integer.valueOf(reqNum), mtranId, notifyMsg.getProvince()[0], priority, Integer.toString(((ContentInfo)contentInfo.get(0)).getContentid().intValue()), true);
