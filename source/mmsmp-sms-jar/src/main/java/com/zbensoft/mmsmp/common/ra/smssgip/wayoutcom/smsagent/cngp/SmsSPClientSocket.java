...
public class SmsSPClientSocket extends ClientSocket {
    ...
    protected int sendtoSMSL(Object objSMS, Socket sc) {
        ...
        if (sc == null) {
            ...
            CNGP_Command comm = (CNGP_Command)objSMS;
    ...
    private boolean readResp(Socket sc) {
        try {
            CNGP_Command resp = CNGP_Command.read(sc);
            if (resp == null) {
                Thread.sleep(1000L);
                return true;
            } else {
    ...
    private void dealResp(CNGP_Command resp, Socket sc) throws IOException {
        int requestID = resp.getCommandID();
        switch(requestID) {
            case -2147483646:
                CNGP_SubmitResp com = (CNGP_SubmitResp)resp;
                ...
                synchronized(SmsProcess.submitQueue) {
                    ...
                    while(it.hasNext()) {
                        ...
                        CNGP_Command submit = key.getSubmit();
                ...
                break;
            case 3:
                CNGP_Deliver deliver = (CNGP_Deliver)resp;
                CNGP_DeliverResp deliverResp = new CNGP_DeliverResp(deliver, 0);
                deliverResp.write(sc);
                this.DownHandler.receive(deliver);
                break;
            case 4:
                CNGP_ActiveTestResp activeResp = new CNGP_ActiveTestResp((CNGP_ActiveTest)resp);
                activeResp.write(sc);
                break;
            case 6:
                CNGP_UnbindResp unbindResp = new CNGP_UnbindResp((CNGP_Unbind)resp);
                unbindResp.write(sc);
                this.activeTestThread.setActiveFalse();
        }
    ...
    protected boolean Login(Socket sc) {
        CNGP_Bind cmd_Bind = null;
        CNGP_BindResp cmd_BindResp = null;
        ...
        cmd_Bind = new CNGP_Bind(clientId, shared_Secret, loginMode, new Date(), version);

        try {
            cmd_Bind.write(sc);
            cmd_BindResp = (CNGP_BindResp)CNGP_Command.read(sc);
    ...
    protected void sendActiveTest() throws Exception {
        (new CNGP_ActiveTest()).write(this.sc);
    }
}
