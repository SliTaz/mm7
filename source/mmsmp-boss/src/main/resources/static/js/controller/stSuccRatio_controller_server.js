'use strict';
var App = angular.module('stSuccRationModule', [ 'datatables',
...
function ServerSideCtrl(DTOptionsBuilder, DTColumnBuilder, $translate, $scope,
		$filter, $compile, StSuccRationService, localStorageService,
		topleftService) {
	var vm = this;
	vm.modelTitle = "";
	vm.readonlyID = false;
	...
	vm.dtColumns = [
			DTColumnBuilder.newColumn('spInfoId').withTitle($translate('stSuccRatio.spInfoId')).notSortable(),
			DTColumnBuilder.newColumn('productName').withTitle($translate('stSuccRatio.productName')).notSortable(),
			DTColumnBuilder.newColumn('productInfoId').withTitle($translate('stSuccRatio.productInfoId')).notSortable(),
...
//	}
	...
	function selData(){
		StSuccRationService.selSpInfo().then(function(d) {
	...
	function submit() {
		if (!vm.readonlyID) {
			...
			StSuccRationService.createUserRecv(vm.bean).then(
			...
			StSuccRationService.updateUserRecv(vm.bean,vm.bean.userRecvId).then(onSubmitSuccess,
					function(errResponse) {
						handleAjaxError(errResponse);
					...
					buttons : [
							{
								...
								action : function(dialogItself) {
									StSuccRationService.deleteUserRecv(bean.userRecvId).then(reloadData,
													function(errResponse) {
														handleAjaxError(errResponse);
	...
	function reloadData() {
		...
		vm.dtInstance.changeData(getFromSource(apiUrl
				+ '/stSuccRatio?spInfoId=' + getValueForSelect(spInfoId) + "&productInfoId=" + getValueForSelect(productInfoId) + "&stTimeStart=" + getValueForSelect(stTimeStart)
