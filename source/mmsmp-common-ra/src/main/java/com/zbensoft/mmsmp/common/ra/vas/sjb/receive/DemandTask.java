...
import com.zbensoft.mmsmp.common.ra.vas.sjb.data.DemandDao;
import com.zbensoft.mmsmp.common.ra.vas.sjb.data.DemandDto;
import com.zbensoft.mmsmp.common.ra.vas.sjb.data.LogDao;
...
public class DemandTask implements Task {
	...
	public void doTask(AbstractMessage message) {
		if ((message instanceof MO_SMMessage)) {
			...
			} else if ((dealType == 1) && (message.getSourceType() == 1)) {
				this.logDao.saveMoSmsMsg(message, null);
			}

		} else if ((message instanceof MO_MMDeliverMessage)) {
			log.info("recevice a new MO_MMDeliverMessage===========================");
			...
			String sendto = me.getTo();
			sendto = sendto.replace("[", "");
			sendto = sendto.replace("]", "");
			log.info("sendto:" + sendto);
			SendNotificationMessage notifyMsg = new SendNotificationMessage();
			notifyMsg.setSendType(100);
			DemandDto dto = this.demandDao.getDemandDto(sendto);

			String str_phone = user_number;
			if (user_number.startsWith("+86"))
				str_phone = user_number.substring(3);
			else if (user_number.startsWith("86"))
				str_phone = user_number.substring(2);
			...
			if ((dto != null) && (dto.getServiceid() != null) && (!"".equals(dto.getServiceid()))) {
				...
				notifyMsg.setLinkId(System.currentTimeMillis()+"");

				if ((retruecode == null) || (retruecode.equals("")) || (retruecode.equals("null"))) {
					retruecode = "1";
					notifyMsg.setSendContent(this.demandDao.queryErrorMsgTemplate() + "Vac鉴权失败,返回错误：" + cresponse.getServerResult_code());
				} else {
					...
					if ((linkid != null) && (linkid.length() > 0) && (!"NULL".equalsIgnoreCase(linkid))) {
						...
						notifyMsg.setSendContent(this.demandDao.queryErrorMsgTemplate() + "Vac鉴权失败---未能获取到LinkID,返回错误：" + cresponse.getServerResult_code());
				...
				log.info("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<usernumber is: " + notifyMsg.getPhoneNumber()[0]);
	...
	private void dispatchMessage(MO_ReportMessage m) {
		...
		log.info("=====>MTTRANID:" + MTTRANID);
		String spreporturl = this.demandDao.getSp_url(MTTRANID);
		log.info("=====>spreporturl:" + spreporturl);
		String newmessageid = this.demandDao.getNewmessageid(MTTRANID);
		...
		try {
			log.info("=====>spreporturl:" + spreporturl);
			String reportstr = reportStr(ush.getMttranid(), ush.getServiceCode(), ush.getCellphonenumber(), Ttime(), newmessageid);
	...
	private String reportStr(String transactionid, String sendnumber, String recipientnumber, String timestamp, String messageid) {
		String report = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><env:Envelope xmlns:env=\"http://schemas.xmlsoap.org/soap/envelope/\"><env:Header><mm7:TransactionID xmlns:mm7=\"http://www.3gpp.org/ftp/Specs/archive/23_series/23.140/schema/REL-6-MM7-1-0\" env:mustUnderstand=\"1\">"
				+ transactionid
				+ "</mm7:TransactionID></env:Header><env:Body><DeliveryReportReq xmlns=\"http://www.3gpp.org/ftp/Specs/archive/23_series/23.140/schema/REL-6-MM7-1-0\"><MM7Version>6.3.0</MM7Version><Sender><Number>"
				+ sendnumber + "</Number></Sender><Recipient><Number>" + recipientnumber + "</Number></Recipient><TimeStamp>" + timestamp + "</TimeStamp><MMSRelayServerID>600003</MMSRelayServerID><MessageID>" + messageid
				+ "</MessageID><MMStatus>Third:User Retrieved</MMStatus><StatusText>1000</StatusText></DeliveryReportReq></env:Body></env:Envelope>";
		return report;
	}
	...
	private boolean isAnswerRight(AnswerRecord answer, String moText) {
		if ((compareDate(answer.getBeginDate(), new Date()) <= 0) && (compareDate(new Date(), answer.getEndDate()) <= 0)) {
