...
public abstract class SMGP_Command {
    ...
    public static SMGP_Command read(Socket sc) throws IOException, Exception {
            ...
            if (pac_len >= head.length && pac_len <= 65536) {
                ...
                if (count < body.length) {
                    ...
                    switch(TypeConvert.byte2int(packet, 4)) {
                        case -2147483647:
                            return new SMGP_BindResp(ByteBuffer.wrap(packet));
                        case -2147483646:
                            return new SMGP_SubmitResp(ByteBuffer.wrap(packet));
                        case -2147483645:
                            return new SMGP_DeliverResp(ByteBuffer.wrap(packet));
                        case -2147483644:
                            return new SMGP_ActiveTestResp(ByteBuffer.wrap(packet));
                        case 1:
                            return new SMGP_Bind(ByteBuffer.wrap(packet));
                        case 2:
                            return new SMGP_Submit(ByteBuffer.wrap(packet));
                        case 3:
                            return new SMGP_Deliver(ByteBuffer.wrap(packet));
                        case 4:
                            return new SMGP_ActiveTest(ByteBuffer.wrap(packet));
                        default:
                            return null;
                    }
                }
